...
export async function runWorkshopToolCall(toolDef, args, _invokeOpts = {}) {
  ...
  //
  // AI-editor checklist (copy into your prompt):
  // 1) Implement `method`, `headers`, `withQuery()`, `callTool()`, and `normalize()`.
  // 2) Make initial request for all tools (free + paid).
  // 3) If status is 402, build a `challenge` object from headers/body.
  // 4) Retry with `fetchWithMpp(...)` when session exists (preferred for mppx backends).
  // 5) Else retry with `_invokeOpts.mintSpt(...)` and payment headers (legacy fallback).
  // 6) Return stable shape: { success, http_status, mode, mpp, response_body, error?, payment_receipt? }.
  return {
    success: false,
    http_status: 0,
    mode,
