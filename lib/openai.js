...
If the error mentions **Stripe profile**, **network business profile**, **issued token**, **API key** (in the SPT / shared payment context), or the failure clearly happened during **SPT mint** (not HTTP 402 from the property API), the fix is usually **\`.env\` on the agent**: set **\`MPP_SELLER_NETWORK_BUSINESS_PROFILE\`** to a \`profile_…\` for the same Stripe account as **\`STRIPE_SECRET_KEY\`** (see \`npm run wallet:print-profile\`), or set **\`MPP_SPT_MINT_MODE=test_helper\`**. Do **not** only repeat “add a card” when a card is already on file.
