 ...
 * - STRIPE_PROFILE_ID — for issued mode
...
function mppSellerProfile() {
  const explicit = (process.env.STRIPE_PROFILE_ID || '').trim();
...
export async function mintSharedPaymentTokenForMppChallenge(session, challenge = {}) {
  const networkId = (challenge.networkId || '').trim() || mppSellerProfile();
...
async function mintSharedPaymentTokenCore(session, opts) {
  ...
  try {
    if (mode === 'test_helper') {
      ...
      if (!profile) {
        throw new Error(
          'Issued-token mint needs networkId from MPP challenge or STRIPE_PROFILE_ID.'
        );
      }
    ...
    if (mode === 'issued') {
      throw new Error(
        `${msg} — Set STRIPE_PROFILE_ID for this Stripe account, or MPP_SPT_MINT_MODE=test_helper.`
      );
    }
