...
const DEFAULT_EVENT_ID = (process.env.HACKATHON_EVENT_ID || '').trim();
...
export function buildJudgePayload(negResult, session) {
  ...
  const eventIdRaw =
    typeof negResult.event_id === 'string'
      ? negResult.event_id
      : typeof negResult.eventId === 'string'
        ? negResult.eventId
        : DEFAULT_EVENT_ID;
  const eventId = eventIdRaw.trim() || 'default';
  ...
  return {
    ...
    transcript: transcript.map((t) => ({
      ...
      speaker_label: t.speakerLabel ?? t.speaker_label,
    })),
    buyer_mandate: typeof negResult.buyerGoal === 'string' ? negResult.buyerGoal.trim() : '',
    buyer_name: typeof negResult.buyerName === 'string' ? negResult.buyerName.trim() : '',
